...
class DocumentProcessingDataExtractorTest < ActiveSupport::TestCase
  ...
  test "rimuove duplicati (testa .uniq)" do
    service = DocumentProcessing::DataExtractor.new(llm_service: DocumentProcessing::LlmService.new(bedrock_client: mock_bedrock('{"recipients":[{"name":"Mario Rossi"},{"name":"Mario Rossi"}]}')))
  ...
  test "ignora nomi troppo corti - normalize_name scarta < 3 caratteri" do
    service = DocumentProcessing::DataExtractor.new(llm_service: DocumentProcessing::LlmService.new(bedrock_client: mock_bedrock('{"recipients":[{"name":"A"},{"name":"Mario Rossi"}]}')))
    assert_equal ["Mario Rossi"], service.extract("testo documento")[:recipients]
  end

  test "normalizza spazi multipli - normalize_name pulisce il nome" do
    service = DocumentProcessing::DataExtractor.new(llm_service: DocumentProcessing::LlmService.new(bedrock_client: mock_bedrock('{"recipients":[{"name":"Mario   Rossi"}]}')))
    assert_equal ["Mario Rossi"], service.extract("testo documento")[:recipients]
  end
  ...
  test "estrae anche data, azienda e reparto" do
    ...
    service = DocumentProcessing::DataExtractor.new(llm_service: DocumentProcessing::LlmService.new(bedrock_client: mock_bedrock(response)))
  ...
  test "ritorna struttura vuota se il testo in input e' vuoto" do
    service = DocumentProcessing::DataExtractor.new(llm_service: DocumentProcessing::LlmService.new(bedrock_client: uncalled_bedrock))
  ...
  test "estrae JSON anche se llm aggiunge testo introduttivo" do
    ...
    service = DocumentProcessing::DataExtractor.new(llm_service: DocumentProcessing::LlmService.new(bedrock_client: mock_bedrock(risposta_con_testo)))
    assert_equal ["Mario Rossi"], service.extract("testo documento")[:recipients]
  end
  ...
  test "ritorna struttura vuota se il client Bedrock solleva un'eccezione" do
    ...
    service = DocumentProcessing::DataExtractor.new(llm_service: DocumentProcessing::LlmService.new(bedrock_client: failing_bedrock(errore)))
  ...
  test "ritorna struttura vuota se la risposta llm non contiene JSON valido" do
    service = DocumentProcessing::DataExtractor.new(llm_service: DocumentProcessing::LlmService.new(bedrock_client: mock_bedrock("risposta senza json")))
    assert_equal [], service.extract("testo documento")[:recipients]
  end

  test "ritorna struttura vuota se la risposta llm e' JSON malformato" do
    service = DocumentProcessing::DataExtractor.new(llm_service: DocumentProcessing::LlmService.new(bedrock_client: mock_bedrock("{recipients: broken")))
    assert_equal [], service.extract("testo documento")[:recipients]
  end
end